  ...
  container_definitions = jsonencode([
    {
      ...
      command = [
        "dbt deps && /work/data_flow_dbt/scripts/apply_elementary_patches.sh && dbt run && dbt test && dbt build --select elementary && mkdir -p /work/data_flow_dbt/elementary_reports/monitoring-reports && edr monitor report --config-dir /work/data_flow_dbt --profiles-dir /work/.dbt --project-dir /work/data_flow_dbt --profile-target dev --days-back 7 --target-path /work/data_flow_dbt/elementary_reports/monitoring-reports && aws s3 sync /work/data_flow_dbt/elementary_reports/monitoring-reports/ s3://${aws_s3_bucket.this.id}/processed/elementary-reports/latest/ --delete && aws s3 cp /work/data_flow_dbt/elementary_reports/monitoring-reports/elementary_report.html s3://${aws_s3_bucket.this.id}/processed/elementary-reports/latest/index.html"
      ]
