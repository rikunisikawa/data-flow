...
- このリポジトリには skills の下書きが `agent-skills-draft/skills/` にある。
...
## 2. skills 配置（ローカル）
下書きがある場合は、`~/.codex/skills/` にコピーする。

```bash
mkdir -p ~/.codex/skills
cp -R agent-skills-draft/skills/* ~/.codex/skills/
```
...
## 5. 本リポジトリでの運用方針
- skills の編集は `agent-skills-draft/skills/` で行う。
